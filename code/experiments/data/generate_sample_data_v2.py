...
class VoltagePatternGenerator:
    ...
    def generate_seasonal_pattern(
        self, n_samples: int, season: str = "summer"
    ) -> np.ndarray:
    ...
    def generate_random_fluctuation(
        self, n_samples: int, std: float = 0.02
    ) -> np.ndarray:
...
class ThreePhaseVoltageGenerator:
    ...
    def generate_balanced_voltage(
        self, n_samples: int, noise_std: float = 2.0, natural_unbalance: float = 0.02
...
class CurrentGenerator:
    ...
    def generate_current(
        ...
        Ia = self.base_current * load_factor * (Va / 220.0) + np.random.normal(
            0, 0.5, n_samples
        )
        Ib = self.base_current * load_factor * (Vb / 220.0) + np.random.normal(
            0, 0.5, n_samples
        )
        Ic = self.base_current * load_factor * (Vc / 220.0) + np.random.normal(
            0, 0.5, n_samples
        )
...
class PowerQualityMetricsGenerator:
    ...
    def generate_quality_metrics(
        Va: np.ndarray, Vb: np.ndarray, Vc: np.ndarray, anomalous_thd: bool = False
        ...
        V_max_dev = np.maximum(
            np.abs(Va - V_mean), np.maximum(np.abs(Vb - V_mean), np.abs(Vc - V_mean))
        )
...
class AnomalyInjector:
    ...
    def inject_voltage_sag(
        ...
        for i in range(start, end):
            ...
            phase_affected = np.random.choice(
                [0, 1, 2], size=np.random.randint(1, 4), replace=False
            )
...
class RuralVoltageDatasetGenerator:
    ...
    def generate_dataset(self, output_dir: str):
        ...
        print(
            f"  Normal: {np.sum(test_labels == 0)} samples ({np.sum(test_labels == 0) / len(test_labels) * 100:.2f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--train_samples", type=int, default=50000, help="Number of training samples"
    )
    parser.add_argument(
        "--test_samples", type=int, default=10000, help="Number of test samples"
    )
    parser.add_argument(
        "--anomaly_ratio",
        type=float,
