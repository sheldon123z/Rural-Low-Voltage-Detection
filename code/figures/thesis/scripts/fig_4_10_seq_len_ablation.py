...
基于 Optuna 30-trial 超参数搜索结果，展示 VoltageTimesNet_v2 在不同
...
import numpy as np
from thesis_style import setup_thesis_style, TIMESNET_COLORS, \
...
def main():
    ...
    # ============================================================
    # VoltageTimesNet_v2 在 RuralVoltage 上的序列长度消融数据
    # 来自 Optuna 30-trial 搜索
    # ============================================================
    seq_lens = [50, 100, 200]
    best_f1_v2 = [0.8149, 0.6750, 0.6623]

    # 绘制 VoltageTimesNet_v2 曲线
    ax.plot(seq_lens, best_f1_v2, 'o-', color=TIMESNET_COLORS[2],
            linewidth=2.0, markersize=8, label='VoltageTimesNet_v2',
            markeredgecolor='white', markeredgewidth=0.8)

    # 标注最优点
    best_idx = np.argmax(best_f1_v2)
    best_seq = seq_lens[best_idx]
    best_f1 = best_f1_v2[best_idx]

    ax.scatter([best_seq], [best_f1], s=150, color=TIMESNET_COLORS[2],
               marker='*', zorder=5, edgecolors='black', linewidths=0.8)
    ax.annotate(f'最优: seq_len={best_seq}\nF1={best_f1:.4f}',
                xy=(best_seq, best_f1),
                xytext=(best_seq + 30, best_f1 - 0.03),
                fontsize=9, color=TIMESNET_COLORS[2],
                arrowprops=dict(arrowstyle='->', color=TIMESNET_COLORS[2], lw=1.0))

    # 标注其他数据点的值
    for i, (sl, f1) in enumerate(zip(seq_lens, best_f1_v2)):
        if i != best_idx:
            ax.annotate(f'{f1:.4f}',
                        xy=(sl, f1),
                        xytext=(sl + 10, f1 + 0.015),
                        fontsize=8.5, color=TIMESNET_COLORS[2])

    # 添加试验次数标注
    trial_counts = [20, 5, 5]
    for sl, f1, tc in zip(seq_lens, best_f1_v2, trial_counts):
