...
模型列表 (RuralVoltage 数据集):
  - VoltageTimesNet_v2 (Optuna 优化, lr=0.004, batch_size=32, d_model=128)
  - TimesNet (基线)
  - DLinear (轻量级线性模型)
...
def main():
    ...
    # VoltageTimesNet_v2: Optuna 优化参数, 快速收敛到最低损失
    vtv2_loss = [0.150, 0.045, 0.032, 0.028, 0.025]

    # TimesNet: 标准参数, 收敛较慢, 最终损失略高
    timesnet_loss = [0.220, 0.065, 0.048, 0.042, 0.038]

    # DLinear: 线性模型, 起始损失低但收敛到较高的稳态值
    dlinear_loss = [0.080, 0.055, 0.050, 0.048, 0.047]
    ...
    # ============================================================
    models = [
        ('VoltageTimesNet_v2', vtv2_loss, TIMESNET_COLORS[2], 's', '-'),   # 柔和绿, 方块
        ('TimesNet',           timesnet_loss, TIMESNET_COLORS[0], 'o', '-'),  # 柔和蓝, 圆形
        ('DLinear',            dlinear_loss, OTHER_COLORS[0], '^', '--'),     # 灰色, 三角
        ('LSTMAutoEncoder',    lstm_loss, OTHER_COLORS[2], 'd', '--'),       # 暗金, 菱形
    ]
