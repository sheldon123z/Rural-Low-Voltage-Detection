...
"""
时序异常检测实验结果分析与可视化 (V2)
使用 FontProperties 显式指定字体，确保中文正确显示
"""
...
if Path(CHINESE_FONT_PATH).exists():
    ...
    FONT_PROP_TITLE = FontProperties(fname=CHINESE_FONT_PATH, size=14, weight='bold')
    FONT_PROP_LABEL = FontProperties(fname=CHINESE_FONT_PATH, size=12)
    FONT_PROP_TICK = FontProperties(fname=CHINESE_FONT_PATH, size=10)
    print(f"✓ 中文字体加载成功: {CHINESE_FONT_PATH}")
else:
    FONT_PROP = None
...
def plot_model_comparison(df, output_dir):
    ...
    for idx, dataset in enumerate(['PSM', 'RuralVoltage']):
        ...
        if len(data) == 0:
            ax.text(0.5, 0.5, f'No {dataset} data', ha='center', va='center', fontsize=14)
            ax.set_title(f'{dataset} Dataset', fontproperties=FONT_PROP_TITLE)
            continue
        ...
        ax.set_xlabel('模型', fontproperties=FONT_PROP_LABEL)
        ax.set_ylabel('F1-score', fontproperties=FONT_PROP_LABEL)
        ax.set_title(f'{dataset} 数据集 - 模型性能对比', fontproperties=FONT_PROP_TITLE)
...
def plot_radar_chart(df, output_dir):
    ...
    for idx, dataset in enumerate(['PSM', 'RuralVoltage']):
        ...
        if len(data) == 0:
            ax.set_title(f'{dataset} - No Data', fontproperties=FONT_PROP_TITLE)
            continue
        ...
        ax.set_xticklabels(metric_labels, fontproperties=FONT_PROP_TICK)
        ax.set_ylim(0, 1.05)
        ax.set_title(f'{dataset} 数据集\n综合性能雷达图', fontproperties=FONT_PROP_TITLE, pad=20)
...
def plot_f1_heatmap(df, output_dir):
    ...
    cbar.set_label('F1-score', fontproperties=FONT_PROP_LABEL)

    ax.set_xlabel('数据集 / 序列长度', fontproperties=FONT_PROP_LABEL)
    ax.set_ylabel('模型', fontproperties=FONT_PROP_LABEL)
    ax.set_title('F1分数热力图 - 模型与配置对比', fontproperties=FONT_PROP_TITLE)
...
def plot_seq_len_analysis(df, output_dir):
    ...
    for idx, dataset in enumerate(['PSM', 'RuralVoltage']):
        ...
        for model in models:
            ...
            if sum(f1_by_seq) > 0:
                color = COLORS.get(model, '#999999')
                ax.bar(x + offset, f1_by_seq, width, label=model, color=color, edgecolor='black')
                offset += width

        ax.set_xlabel('序列长度 (seq_len)', fontproperties=FONT_PROP_LABEL)
        ax.set_ylabel('F1-score', fontproperties=FONT_PROP_LABEL)
        ax.set_title(f'{dataset} 数据集 - 序列长度影响分析', fontproperties=FONT_PROP_TITLE)
...
def plot_multi_metric_comparison(df, output_dir):
    ...
    for i, (metric, label) in enumerate(zip(metrics, metric_labels)):
        ...
        ax.set_xlabel('模型', fontproperties=FONT_PROP_LABEL)
        ax.set_ylabel(label, fontproperties=FONT_PROP_LABEL)
        ax.set_title(f'{label}对比', fontproperties=FONT_PROP_TITLE)
