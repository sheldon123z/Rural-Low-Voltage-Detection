...
class PresetPeriodBlock(nn.Module):
    ...
    def __init__(self, configs, preset_periods: List[int]):
        ...
        valid_periods = [p for p in preset_periods if p >= 2 and p <= configs.seq_len // 2]
...
class ConfidenceBasedFusion(nn.Module):
    ...
    def forward(self, preset_out: torch.Tensor, fft_out: torch.Tensor,
                preset_conf: torch.Tensor, fft_conf: torch.Tensor) -> torch.Tensor:
        combined = (preset_out + fft_out) / 2
        ctx = combined.mean(dim=1)
        weights = self.confidence_mlp(ctx)
        
        alpha = torch.sigmoid(self.alpha)
        final_weights = alpha * weights + (1 - alpha) * torch.tensor([0.5, 0.5], device=weights.device)
        
        weights_expanded = final_weights.unsqueeze(1).unsqueeze(1)
        
        output = weights_expanded[:, :, :, 0:1] * preset_out + weights_expanded[:, :, :, 1:2] * fft_out
