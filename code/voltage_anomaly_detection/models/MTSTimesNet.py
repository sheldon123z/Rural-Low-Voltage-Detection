...
class ScaleSpecificTimesBlock(nn.Module):
    ...
    def __init__(self, configs, scale_name: str = 'medium', min_period: int = 10, max_period: int = 50):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        period_list, period_weight = FFT_for_Period_Range(x, self.k, self.min_period, self.max_period)
...
class CrossScaleConnection(nn.Module):
    ...
    def __init__(self, d_model: int, n_scales: int = 3):
        ...
        self.n_scales = n_scales
        self.cross_attention = nn.MultiheadAttention(d_model, num_heads=4, dropout=0.1, batch_first=True)
        self.projections = nn.ModuleList([nn.Linear(d_model, d_model) for _ in range(n_scales)])
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        for scale_name, scale_cfg in self.SCALE_CONFIGS.items():
            self.scale_blocks[scale_name] = nn.ModuleList([
                ScaleSpecificTimesBlock(configs, scale_name=scale_name,
                    min_period=scale_cfg['min_period'], max_period=scale_cfg['max_period'])
    ...
    def _adjust_scale_configs(self):
        ...
        valid_scales = {k: v for k, v in self.SCALE_CONFIGS.items() if v['min_period'] < v['max_period']}
        
        if len(valid_scales) < 3:
            self.SCALE_CONFIGS = {
                'short': {'min_period': 2, 'max_period': max(4, seq_len // 10)},
                'medium': {'min_period': max(4, seq_len // 10), 'max_period': max(8, seq_len // 5)},
                'long': {'min_period': max(8, seq_len // 5), 'max_period': seq_len // 2},
            }
