...
class ThreePhaseAttention(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, phase_indices: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        B, T, C = x.size()
        residual = x
...
class PhaseAwareTimesBlock(nn.Module):
    ...
    def __init__(self, configs, use_phase_attention: bool = True):
        ...
        self.fusion_gate = nn.Sequential(
            nn.Linear(configs.d_model * 2, configs.d_model), nn.Sigmoid()
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.k):
            ...
            if (self.seq_len + self.pred_len) % period != 0:
                ...
                padding = torch.zeros(
                    [B, length - (self.seq_len + self.pred_len), N], device=x.device
                )
