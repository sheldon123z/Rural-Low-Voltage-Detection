...
Note: This model is self-contained and doesn't depend on external layers.
...
def transition(N):
    """Create HiPPO transition matrices."""
...
class HiPPO_LegT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for f in inputs.permute([-1, 0, 1]):
            ...
            c = F.linear(c, self.A.to(device)) + new
            cs.append(c)
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec_true, x_mark_dec):
        return self._core_forward(x_enc)
