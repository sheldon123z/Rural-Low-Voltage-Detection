...
class StatisticalFeatures:
    ...
    def extract(
        ...
        for col in columns:
            for ws in self.window_sizes:
                # Rolling mean
                result[f"{col}_mean_{ws}"] = (
                    data[col].rolling(window=ws, min_periods=1).mean()
                )

                # Rolling std
                result[f"{col}_std_{ws}"] = (
                    data[col].rolling(window=ws, min_periods=1).std().fillna(0)
                )
    ...
    def extract_advanced(
        self, data: pd.DataFrame, columns: List[str], window: int = 20
    ) -> pd.DataFrame:
        ...
        for col in columns:
            ...
            # Skewness
            result[f"{col}_skew_{window}"] = (
                series.rolling(window=window, min_periods=1).skew().fillna(0)
            )

            # Kurtosis
            result[f"{col}_kurt_{window}"] = (
                series.rolling(window=window, min_periods=1).kurt().fillna(0)
            )

            # Percentiles
            result[f"{col}_q25_{window}"] = series.rolling(
                window=window, min_periods=1
            ).quantile(0.25)
            result[f"{col}_q75_{window}"] = series.rolling(
                window=window, min_periods=1
            ).quantile(0.75)
...
class FrequencyFeatures:
    ...
    def extract(
        self, data: pd.DataFrame, columns: List[str], window: int = 64
    ) -> pd.DataFrame:
        """
            ...
            for i in range(window, n_samples):
                ...
                if len(pos_magnitudes) > 0:
                    ...
                    spectral_entropy[i] = -np.sum(
                        normalized_mag * np.log(normalized_mag + 1e-8)
                    )
        ...
        for col in voltage_cols:
            if col in data.columns:
                ...
                if thd_col in data.columns:
                    ...
                    result[f"{col}_3rd_harmonic_est"] = (
                        data[thd_col] * 0.6
                    )  # 3rd harmonic typically dominant
...
class SymmetricalComponentFeatures:
    ...
    def extract(
        self, data: pd.DataFrame, voltage_cols: List[str] = ["Va", "Vb", "Vc"]
    ) -> pd.DataFrame:
        """
...
class FeatureEngineer:
    ...
    def transform(
        ...
        if self.enable_frequency:
            ...
            if all(col in data.columns for col in voltage_cols):
                ...
                result = self.freq_extractor.extract_harmonic_features(
                    result, voltage_cols
                )

        if self.enable_interphase:
            print("Extracting inter-phase features...")
            result = self.interphase_extractor.extract(
                result, voltage_cols, current_cols
            )
    ...
    def get_feature_names(self, original_columns: List[str]) -> List[str]:
        ...
        dummy = pd.DataFrame(
            np.random.randn(100, len(original_columns)), columns=original_columns
        )
...
def main():
    ...
    engineer = FeatureEngineer(stat_window_sizes=[10, 20], freq_window=32)
